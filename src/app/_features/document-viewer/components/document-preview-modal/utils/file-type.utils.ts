import { getFileExtension } from '@/_shared/lib/file.utils';

export type FileType = 'image' | 'pdf' | 'video' | 'audio' | 'text' | 'unsupported';
...
const TEXT_EXTENSIONS = ['txt', 'md', 'json', 'xml', 'yaml', 'yml', 'js', 'ts', 'jsx', 'tsx', 'css', 'html'];
...
export function detectFileType(fileName: string, mimeType: string): FileTypeInfo {
  ...
  if (normalizedMimeType.startsWith('image/') || IMAGE_EXTENSIONS.includes(extension)) {
  ...
  if (normalizedMimeType.startsWith('video/') || VIDEO_EXTENSIONS.includes(extension)) {
    return { type: 'video', extension, mimeType: normalizedMimeType };
  }

  if (normalizedMimeType.startsWith('audio/') || AUDIO_EXTENSIONS.includes(extension)) {
    return { type: 'audio', extension, mimeType: normalizedMimeType };
  }

  if (normalizedMimeType.startsWith('text/') || TEXT_EXTENSIONS.includes(extension)) {
